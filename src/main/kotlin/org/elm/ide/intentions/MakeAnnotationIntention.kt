...
class MakeAnnotationIntention : ElmAtCaretIntentionActionBase<MakeAnnotationIntention.Context>() {
    ...
    data class Context(val valueDeclaration: ElmValueDeclaration, val ty: Ty)
    ...
    override fun findApplicableContext(project: Project, editor: Editor, element: PsiElement): Context? {
        ...
        return Context(declaration, ty)
    }

    override fun invoke(project: Project, editor: Editor, context: Context) {
        val (decl, ty) = context
        val indent = editor.getIndent(decl.startOffset)
        val code = "${decl.name} : ${ty.renderedText(elmFile = decl.elmFile).replace("→", "->")}\n$indent"
        project.runWriteCommandAction {
            editor.document.insertString(decl.startOffset, code)
        }
    }
}
